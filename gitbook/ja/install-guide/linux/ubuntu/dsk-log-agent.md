...
`Log agent`は現在、Ubuntu 22.04(Jammy Jellyfish)、20.04(Focal Fossa)、18.04(Bionic Beaver)、16.04(Xenial Xerus)環境をサポートしています.
...
```
fluent-bitパッケージのインストールの詳細については、`公式文書`（https://docs.fluentbit.io/manual/installation/linux/ubuntu）を参照してください。
